...
Before I can talk about the project, it will be prudent to give a brief rundown of what network science actually is for the uninitiated -- along with the meanings of some of its associated terms.

At its core, network science is the study of _connectivity_. It moves the focus away from the individual characteristics of single entities and toward the relationships _between_ those entities, analyzing the resulting "whole" as one new, consolidated entity: a network, which encodes in its relationships a coarse-grained description of the entire system.
...
Here, at long last, is a full image of the transfer portal in all its glory -- in this case for the most recent year, 2025. Nodes are sized and colored by their degree (number of connections), so that higher-degree schools are larger and bluer. 
...
There is a clear hierarchy between divisions, with D2 and D3 programs at the outer fringes of the network and FBS programs in the center. This reflects a classic hierarchical "out-to-in" flow pattern, where significantly more players transfer up from lower divisions than down into them. (this notably does _not_ hold within FBS, however; we'll explore that later). This pattern makes sense given that there is usually little reason for a player in D2 or D3 to transfer laterally.
...
Using PPA as a proxy for player quality and summing it for each team in each year, we can then take the coefficient of variation (CV) for each year and compare its values before and after the introduction of the portal. Doing this, we observe that parity does in fact seem to have increased since 2021.
...
Combined with the status of "mid-tier" programs as go-betweens for rising stars in lower divisions and FCS, the net effect of the portal indeed seems to be an increase in parity.

- two other takeaways (centrality + topology?)

# Recruiting and Talent Metrics

I promised I'd get to recruiting, so here we are. 



I must admit that I didn't end up getting to do nearly as much with this as I would have liked, 